...
public class WorldContactListener implements ContactListener {
    ...
    public void beginContact(Contact contact) {
        ...
        if((fixA.getUserData() instanceof Hero || fixB.getUserData() instanceof Hero) && (fixA.getUserData() instanceof Enemy || fixB.getUserData() instanceof Enemy)){
            Fixture heroFix = fixA.getUserData() instanceof Hero ? fixA : fixB;
            Hero hero = (Hero) heroFix.getUserData();
            if(!hero.getState().equals(State.INVINCIBLE))
                hero.decrementHP(10);
        }
    }
